...
/**
* Container events
* @param  {Object}   opts     Events options. (optional)
* @param  {Function} callback Callback with data
*/
Container.prototype.events = function(opts, callback) {
  var isStream = !(opts.since && opts.until);
  var optsf = {
    path: '/events?',
    method: 'GET',
    options: extend({}, this.defaultOptions.events, opts),
    isStream: isStream,
    statusCodes: {
      200: true,
      500: "server error"
  ...
  if(isStream){
    ...
    this.modem.dial(optsf, function(err, data) {
      ...
      if(!err){
        var events = data.split(/({[^}]*})/).filter(function(s){return s;}).map(JSON.parse);
      }
      callback(err, events);
    });
  }
